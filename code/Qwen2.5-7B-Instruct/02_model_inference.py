...
def setup_logging():
    """配置日志系统：同时输出到控制台和文件"""
    log_dir = "../results/logs/new_qwen2.5_logs/02_model_inference"
    os.makedirs(log_dir, exist_ok=True)
    
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(log_dir, f"inference_qwen2.5_log_{timestamp}.log")
...
OUTPUT_DIR = "../results/new_qwen2.5_results"
os.makedirs(OUTPUT_DIR, exist_ok=True)
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "inference_results_qwen2.5.json")

# Qwen2.5模型配置
MODEL_NAME = "Qwen/Qwen2.5-7B-Instruct"
MODEL_CACHE_DIR = "/root/autodl-tmp/dongba/model/Qwen2.5-7B-Instruct"
...
# ===================== 模型加载 =====================
def load_qwen2_5_model():
    """加载Qwen2.5-7B-Instruct模型 - 完全修复版"""
    logger.info(f"开始加载Qwen2.5模型，路径: {MODEL_CACHE_DIR}")
    ...
    if missing_files:
        ...
        logger.info("请先运行 qwen2_tokenizer_fix.py 修复文件")
    
    try:
        # 方法1：直接导入Qwen2Tokenizer
        logger.info("尝试导入Qwen2Tokenizer...")
        try:
            from transformers import Qwen2Tokenizer, Qwen2ForCausalLM
            
            logger.info("Qwen2Tokenizer 导入成功，开始加载...")
            ...
            logger.info(f"✓ 使用Qwen2Tokenizer加载成功")
            
        except ImportError as e:
            logger.warning(f"Qwen2Tokenizer导入失败: {str(e)}")
            logger.info("尝试使用AutoTokenizer...")
            
            # 方法2：使用AutoTokenizer（transformers 4.38+ 应该支持）
            ...
            logger.info(f"✓ 使用AutoTokenizer加载成功")
        ...
        logger.info("\n" + "="*60)
        logger.info("故障排除建议:")
        logger.info("1. 确保模型文件完整:")
        logger.info(f"   检查目录: {MODEL_CACHE_DIR}")
        logger.info("2. 确保有以下文件:")
        logger.info("   - config.json")
        logger.info("   - tokenizer_config.json")
        logger.info("   - special_tokens_map.json")
        logger.info("   - tokenizer.json (或类似分词器文件)")
        logger.info("   - model.safetensors (或pytorch_model.bin)")
        logger.info("3. 运行修复脚本:")
        logger.info("   python qwen2_tokenizer_fix.py")
        logger.info("4. 检查Python包版本:")
        logger.info("   pip show transformers tiktoken torch")
        logger.info("="*60)
        
        raise

# ===================== 提示词构建 =====================
def build_qwen2_5_prompt(sample):
    """构建Qwen2.5的提示词"""
    # 提取DB编码和释义
    db_info = []
...
# 你是一个专业的小语种翻译专家，擅长将符号编码转换为自然流畅的中文句子。
...
# ===================== 文本生成 =====================
def generate_with_qwen2_5(model, tokenizer, sample, max_retries=2):
    """使用Qwen2.5生成文本，带重试机制"""
    prompt = build_qwen2_5_prompt(sample)
...
def batch_inference(model, tokenizer, samples):
    ...
    # # 如果样本太多，可以选择部分测试
    # max_samples = min(50, len(samples))  # 限制样本数量进行测试
    # if len(samples) > max_samples:
    #     logger.info(f"样本数较多，限制为前 {max_samples} 个样本进行测试")
    #     samples = samples[:max_samples]
    
    for i, sample in enumerate(tqdm(samples, desc="推理进度")):
        try:
            # 生成文本
            generated_text = generate_with_qwen2_5(model, tokenizer, sample)
...
def main():
    try:
        logger.info("=" * 60)
        logger.info("Qwen2.5-7B-Instruct 模型推理流程开始")
        logger.info("=" * 60)
        ...
        # 2. 加载模型
        model, tokenizer = load_qwen2_5_model()
