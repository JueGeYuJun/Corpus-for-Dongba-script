...
def setup_logging():
    """配置日志系统：同时输出到控制台和文件"""
    log_dir = "../results/logs/bart_logs/03_evaluation"
...
# ===================== 核心配置 =====================
INPUT_DATA_PATH = "../results/new_bart_results/inference_results_bart.json"
OUTPUT_EVAL_DIR = "../results/new_bart_reports"
os.makedirs(OUTPUT_EVAL_DIR, exist_ok=True)
OUTPUT_EVAL_PATH = os.path.join(OUTPUT_EVAL_DIR, "evaluation_report_bart.json")
...
def main():
    try:
        logger.info("="*60)
        logger.info("开始执行bart-base-chinese模型评估流程")
        logger.info("="*60)
        ...
        # 6. 打印核心指标汇总
        logger.info("\n" + "="*60)
        logger.info("bart-base-chinese模型全量基准测试核心指标汇总")
        logger.info("="*60)
        ...
        logger.info("="*60)
        logger.info("bart-base-chinese模型全量基准测试评估执行完成")
        logger.info("="*60)
